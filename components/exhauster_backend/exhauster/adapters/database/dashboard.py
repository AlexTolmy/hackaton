...
class RotorRepo(BaseRepository):
    ...
    def get(self, exhauster_id: int) -> dto.Rotor:
        ...
        stmt = select(tables.rotors
                      ).where(tables.rotors.c.exhauster_id == exhauster_id
                              ).order_by(tables.rotors.c.installed_at.desc()
                                         ).limit(1)
        row = self.session.execute(stmt).one()

        return dto.Rotor(installed_at=row.installed_at, name=row.name)

    def get_prediction(self, exhauster_id: int) -> Optional[datetime]:

        stmt = select(tables.rotors_prediction.c.stop_at).where(
            tables.rotors_prediction.c.exhauster_id == exhauster_id
        ).order_by(tables.rotors_prediction.c.created_at.desc()).limit(1)
